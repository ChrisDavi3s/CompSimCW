...
extern "C" {
    void run_simulation(int32_t systemSize, int32_t mcSweeps, int32_t mcAdjustmentSweeps, int32_t externalField, int32_t tempSampleSize, double *temperatures, double *heatCapacities, double *magnetisations, double *susceptibilities) {

        vector<double> heatCapacitiesVec(tempSampleSize);
        vector<double> magnetisationsVec(tempSampleSize);
        vector<double> susceptibilitiesVec(tempSampleSize);

        CalculatePhysicalQuantities(systemSize, mcSweeps, mcAdjustmentSweeps, externalField, vector<double>(temperatures, temperatures + tempSampleSize), heatCapacitiesVec, magnetisationsVec, susceptibilitiesVec);

        for (int32_t i = 0; i < tempSampleSize; ++i) {
            heatCapacities[i] = heatCapacitiesVec[i];
            magnetisations[i] = magnetisationsVec[i];
            susceptibilities[i] = susceptibilitiesVec[i];
        }
    }
}
...
//do we risk -O3 loll?
//my path file is f**ked up 
//how tf does macOS work